...
function backPackBack(dataName, oldValue)
  if getElementType(source) == "player" and dataName == "MAX_Slots" then
	...
	elseif newValue == 10 then
      elementBackpack[source] = createObject(2384, x, y, z)
	elseif newValue == 12 then
      elementBackpack[source] = createObject(2401, x, y, z)
	elseif newValue == 16 then
      elementBackpack[source] = createObject(2407, x, y, z)
...
if weapon1 == "SCAR-H" then
elementWeaponRaplace[source] = createObject(2962,x,y,z)
setObjectScale(elementWeaponBack[source],0.875)
elseif weapon1 == "AK-74U" then
elementWeaponRaplace[source] = createObject(2963,x,y,z)
setObjectScale(elementWeaponBack[source],0.875)
elseif weapon1 == "M16A2" then
elementWeaponRaplace[source] = createObject(2964,x,y,z)
setObjectScale(elementWeaponBack[source],0.875)
elseif weapon1 == "SIG50" then
elementWeaponRaplace[source] = createObject(2965,x,y,z)
setObjectScale(elementWeaponBack[source],0.875)
elseif weapon1 == "DSR" then
elementWeaponRaplace[source] = createObject(2979,x,y,z)
setObjectScale(elementWeaponBack[source],0.875)
...
function onPlayerRefillWaterBottle(itemName)
  if isElementInWater(source) then
    setElementData(source, "Bouteille", getElementData(source, "Bouteille") + 1)
    setElementData(source, itemName, getElementData(source, itemName) - 1)
    triggerClientEvent(source, "refreshInventoryManual", source)
    triggerClientEvent(source, "displayClientInfo", source, "Bouteille", shownInfos.filledup, 22, 255, 0)
  else
    triggerClientEvent(source, "displayClientInfo", source, "Bouteille", shownInfos.needwatersource, 255, 22, 0)
  end
end
...
function outputInfo3()
  for i, player in ipairs(getElementsByType("player")) do
    triggerClientEvent(player, "displayClientInfo", player, "Info", "COMMANDERZ", 200, 200, 22)
  end
