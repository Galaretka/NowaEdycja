...
--определяет, когда бот меняет статус и начинает новое поведение
function changestatus (dataName)
	if getElementType ( source ) == "ped" and dataName == "status" and (getElementData (source, "slothbot") == true) then
		...
		elseif (getElementData ( source, "status" ) == "hunting" ) then
			local oldPx, oldPy, oldPz = getElementPosition( source )
			setTimer ( enemy_check, 600 ,1 , source )-- начинает сканирование для врагов
		...
		elseif (getElementData ( source, "status" ) == "guarding" ) then
			setTimer ( guard_enemy_check, 800 ,1 , source )-- начинает сканирование для врагов
...
--продолжает цель бота немного отставали
		...
		if (getElementData ( ped, "status" ) ==  "chasing" ) and (isElement(player)) and (getElementData (ped, "slothbot") == true) then
			if getElementType(player) == "player" then
				...
				if pedhp > 0 and (isPedDead(player) == false) then
					...
					if (isclear == true) then -- если цель Игрок в поле зрения
	...
	if (isElement(ped)) then
		...
		if (getElementData ( ped, "status" ) ==  "chasing" ) and (isElement(thetarget)) and (getElementData (ped, "slothbot") == true) then
			...
			if pedhp > 0 and tarhp > 0 then
				...
				setPedAnimation( ped, "ped", "sprint_panic" )
				...
				agr = playSound3D(idle[math.random(1, #idle)], px, py, pz, false)
				setSoundVolume(agr, 0.4)
			end
		end
	end
end
...
--устанавливает вращение и сообщает серверу, когда цель в поле зрения
...
--наборы направлены в режиме наблюдения (возможно избыточный функция, может сочетаться с chase_aim)
...
--ищет цели в режиме наблюдения (работает только для контроллера пед)
...
--рассказывает бота стрелять в режиме наблюдения
...
--функции приведенные ниже, являются для, когда игрок стреляет из пистолета, оно привлекает внимание окружающих пешеходов.
...
--следующие 3 функции установите регуляторы пед когда сервер диктат
...
--эта функция сохраняет пед бежит к все элемент установлен в качестве его "TargetPath" elementdata
...
--это, чтобы исправить проблему, что unstreamed пешеходы потерять все, но 1 пулю для местного игрока
function Streamin ()
			...
			if (getElementData ( ped, "controller" ) == getLocalPlayer()) then
				...
				for theKey,thePlayer in ipairs(enemies) do
					if (isPedDead(thePlayer) == false) then
						...
						local isclear = isLineOfSightClear (Px, Py, Pz+.6, x, y, z +.6, true, false, false, true, false, false, false ) 
						local distance = (getDistanceBetweenPoints3D (Px, Py, Pz, x, y, z))
						if (isclear == true) and (distance < 150) then --если игрок находится в прямой видимости и в зоне
							if getPlayerTeam(thePlayer) == false then
								triggerServerEvent ( "onBotFindEnemy", ped, thePlayer)
								foundone = 1
							elseif (getElementData ( ped, "BotTeam" ) ~= getPlayerTeam (thePlayer)) then
	...
	if (isElement(ped)) then
		if ( getElementData ( ped, "status" ) == "guarding" ) and (getElementHealth(ped)>0) and (getElementData (ped, "slothbot") == true) then
			...
			if (isElement(guardcol)) then
				if (getElementData (ped, "guard_shooting") == true) then -- если пед занят съемки на врага
	...
	if #players == 1 then
		...
		for theKey,thePed in ipairs(peds) do
			...
			elseif (getElementData ( thePed, "status" ) == "hunting" ) then
				local oldPx, oldPy, oldPz = getElementPosition( thePed )
				setTimer ( enemy_check, 600 ,1 , thePed )-- начинает сканирование для врагов
			...
			elseif (getElementData ( thePed, "status" ) == "guarding" ) then
				setTimer ( guard_enemy_check, 800 ,1 , thePed )-- начинает сканирование для врагов
